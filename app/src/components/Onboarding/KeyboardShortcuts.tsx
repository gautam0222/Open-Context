...
export default function KeyboardShortcuts() {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 p-3 bg-brand-600 text-white rounded-full shadow-lg hover:bg-brand-700 transition z-40"
  ...
  const shortcuts = [
    ...
    { key: 'N', action: 'New capture (when available)' },
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
          <p className="text-xs text-gray-600 text-center">
            Press <kbd className="px-2 py-1 bg-white border border-gray-300 rounded text-xs">⌘K</kbd> or{' '}
            <kbd className="px-2 py-1 bg-white border border-gray-300 rounded text-xs">Ctrl+K</kbd> anytime to toggle this menu
          </p>
        </div>
      </div>
    </div>
